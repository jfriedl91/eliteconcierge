...
const tiers = [
  {
    ...
    features: [
      ...
      <span>
        Behördenhilfe &{" "}
        <span className="whitespace-nowrap">KFZ-Angelegenheiten</span>
      </span>,
      "Unterstützung bei Wohnungssuche & WG-Zimmer-Suche",
      <span>
        Geschenk- & <span className="whitespace-nowrap">Event-Planung</span>
      </span>,
    ],
  ...
  },
  {
    ...
    priceSuffix: "/ Monat",
    description:
      "Für Unternehmer & Vielreisende mit hohem Organisationsbedarf.",
    features: [
      ...
      <span>
        Monatlicher <span className="whitespace-nowrap">Strategie-Call</span>
      </span>,
      <span>
        Kontakt via Telefon, <span className="whitespace-nowrap">E-Mail</span>{" "}
        und Messenger
      </span>,
  ...
  },
  {
    ...
    features: [
      ...
      <span>
        Spezialisierter Reise &{" "}
        <span className="whitespace-nowrap">Immobilien-Concierge</span>
      </span>,
  ...
  },
  {
    ...
    features: [
      ...
      <span>
        Business Concierge für{" "}
        <span className="whitespace-nowrap">Backoffice & Koordination</span>
      </span>,
      <span>
        Eigenes <span className="whitespace-nowrap">Backoffice-Team</span>
      </span>,
...
const PricingPage = () => {
  ...
  const handleCreateContract = (packageName) => {
    if (packageName) {
      navigate(
        `/dienstleistungsauftrag?package=${encodeURIComponent(
          packageName
  ...
  const getPriceDisplay = (tier) => {
    if (tier.name === "BESPOKE") {
      ...
      return {
        price:
          tier.price === "Preis auf Anfrage"
            ? tier.price
            : `${tier.priceNet.toLocaleString("de-DE", {
          ...
          tier.price === "Preis auf Anfrage" ? (
            ...
            <span className="leading-tight">
              ...
              <span className="text-xs text-platinum/50 font-normal">
                (
                {grossPrice.toLocaleString("de-DE", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
                € brutto)
    ...
    if (customerType === "business" && tier.priceGross) {
      ...
      return {
        price: `${netPrice.toLocaleString("de-DE", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}€`,
        suffix: (
          <span className="leading-tight">
            ...
            <span className="text-xs text-platinum/50 font-normal">
              (
              {tier.priceGross.toLocaleString("de-DE", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
              € brutto)
    ...
    return {
      price: `${tier.priceGross.toLocaleString("de-DE", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      })}€`,
  ...
  return (
    <PageWrapper>
      <Helmet>
        ...
        <meta
          name="description"
          content="Wählen Sie das Mitgliedschaftspaket, das perfekt zu Ihrem Lebensstil passt: SMART, PRIME, ELITE oder ein vollständig maßgeschneiderter BESPOKE Service."
        />
        ...
        <div className="container mx-auto py-24 px-4">
          ...
          >
            ...
            <p className="text-xl md:text-2xl text-gold font-serif">
              Finden Sie Ihren perfekten Service-Level.
            </p>
          </motion.div>
          ...
          >
            <Label
              ...
              className={`transition-colors ${
                customerType === "private" ? "text-gold" : "text-platinum/70"
              }`}
            >
              Privatkunde
            </Label>
            <Switch
              ...
              onCheckedChange={(checked) =>
                setCustomerType(checked ? "business" : "private")
              }
            ...
            <Label
              htmlFor="customer-type"
              className={`transition-colors ${
                customerType === "business" ? "text-gold" : "text-platinum/70"
              }`}
            >
              Geschäftskunde
            </Label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-5xl mx-auto items-stretch">
            {tiers.map((tier, index) => {
              ...
              return (
                ...
                >
                  <Card
                    className={`bg-secondary rounded-none h-full flex flex-col transition-all duration-300 hover:-translate-y-2.5 ${
                      tier.featured
                        ? "border-2 border-gold transform lg:scale-105 shadow-2xl shadow-gold/20 hover:lg:scale-110"
                        : "border " + tier.borderColor + " hover:scale-103"
                    }`}
                  >
                    <CardHeader className="text-center p-6">
                      <CardTitle
                        className={`text-2xl md:text-3xl font-serif ${
                          tier.featured ? "text-gold" : "text-platinum"
                        }`}
                      >
                        {tier.name}
                      </CardTitle>
                      <CardDescription className="text-platinum/70 min-h-[3rem] pt-1">
                        {tier.description}
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="flex-grow flex flex-col justify-between p-6">
                          ...
                          {tier.features.map((feature, i) => (
                            ...
                            >
                              ...
                              {feature.isFairUse ? (
                                ...
                                <span
                                  className={`text-platinum/90 text-sm ${
                                    i === 0 && tier.name === "SMART"
                      ...
                      <Button
                        ...
                        //     : tier.name === "BESPOKE"
                        //     ? () => handleInquire(tier.name)
                        //     : () => handleCreateContract(tier.packageName)
                        // }
                        onClick={() => handleInquire("Allgemeine Beratung")}
                        className={`w-full mt-8 rounded-none text-base md:text-lg py-4 md:py-6 transition-all duration-300 ${
                          tier.featured
                            ? "bg-gold text-background hover:bg-platinum hover:shadow-lg hover:shadow-gold/50"
                            : "bg-transparent border border-gold/50 text-gold/80 hover:bg-gold hover:text-background hover:border-gold"
                        }`}
                      >
                        {/* {tier.isSmart
                          ? "Details anzeigen"
          ...
          >
            ...
            <p className="mt-4 text-base">
              Keine langen Vertragslaufzeiten, keine Verpflichtungen. Alle
              unsere Mitgliedschaften sind monatlich mit einer Frist von nur
              zwei Wochen zum Monatsende kündbar. Genießen Sie volle
          ...
          >
            ...
            <p className="mt-4 text-base max-w-xl mx-auto text-platinum/80">
              Sie sind unsicher, welches Paket das richtige für Sie ist? Wir
              beraten Sie gerne. Kontaktieren Sie uns für ein unverbindliches
              Erstgespräch.
            </p>
            <Button
              onClick={() => handleInquire("Allgemeine Beratung")}
          ...
          >
            <SmartPackagesTable
              onInquire={handleCreateContract}
              customerType={customerType}
            />
          </motion.div>
          ...
          >
            ...
            <p className="text-lg text-platinum/80 max-w-2xl mx-auto mb-8">
              Erstellen Sie jetzt direkt Ihren persönlichen
              Dienstleistungsauftrag und werden Sie Teil des exklusiven
              ELCONCI-Netzwerks.
            </p>
            <Link to={`/dienstleistungsauftrag?customerType=${customerType}`}>
              <Button
                size="lg"
                className="bg-gold text-background hover:bg-platinum rounded-none"
              >
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-3xl p-0 flex flex-col w-[95%] sm:w-full max-h-[90vh] sm:max-h-[85vh]">
          <DialogHeader className="p-6 pb-4 flex-shrink-0">
            <DialogTitle>
              Anfrage für eine Elite Concierge Mitgliedschaft
            </DialogTitle>
            <DialogDescription>
              Füllen Sie das Formular aus, um Ihr Interesse zu bekunden. Wir
