...
const SmartPackagesTable = ({ onInquire, customerType, promoDiscount }) => {
  const getPrice = (priceGross) => {
    ...
    const discountedPriceGross = promoDiscount
      ? priceGross - promoDiscount
      : priceGross;
    ...
    if (customerType === "business") {
      ...
      return (
        <div className="text-right">
          {promoDiscount && (
            <span className="text-base line-through text-platinum/50 mr-2">
              {originalNetPrice.toLocaleString("de-DE", {
          ...
          <p className="text-xs text-platinum/50 font-normal">
            zzgl. USt. / {promoDiscount ? "1. Monat" : "Monat"}
          </p>
          <p className="text-xs text-platinum/50 font-normal">
            (
            {discountedPriceGross.toLocaleString("de-DE", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
            € inkl. USt.)
    ...
    return (
      <div className="text-right">
        {promoDiscount && (
          <span className="text-base line-through text-platinum/50 mr-2">
            {originalPriceGross.toLocaleString("de-DE", {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
            €
        ...
        <p className="text-xs text-platinum/50 font-normal">
          inkl. MwSt. / {promoDiscount ? "1. Monat" : "Monat"}
        </p>
      </div>
    );
  };
